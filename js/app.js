...
app.controller('MainCtrl',['$scope', '$filter', '$http' , function($scope, $filter, $http){
    ...
    $scope.excel = function(checked){
        if(checked) {
            //flink_sender = JSON.stringify($scope.flinks);
            console.log(JSON.stringify($scope.flinks));
            $http.post('excel.php', {data: JSON.stringify($scope.flinks)}).success(function (output) {
                console.log(output);
